...
export async function getConnector(type: SourceType, p: Queryable = db()): Promise<ConnectorRow | null> {
  const { rows } = await p.query(
  ...
  if (!row) return null;
  return { ...row, credentials: await decryptStoredCredentials(row.credentials) };
}
...
export async function upsertConnector(
  ...
  const creds = patch.credentials !== undefined
