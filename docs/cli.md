...
The package is ESM, requires Node.js 22+, has zero runtime dependencies, and exposes the `brian` binary. Node 20 reached end of life before this release, so the plan's “Node 20+ or oldest actively supported version” rule resolves to Node 22. Version `0.1.1` fixes Claude Desktop setup by using its account-level custom connector flow instead of writing an invalid remote entry to the local-server configuration.
...
- `connect` detects clients, validates every selected config first, backs up existing files, and writes the canonical URL-only entry for file-configured clients before offering native authentication. For Claude Desktop it removes an obsolete Brian local-config entry, if present, and prints the account-level custom connector steps. Native commands run sequentially after a separate confirmation. A login failure leaves the valid configuration installed and reports a fixed retry command. A second configuration run is byte-idempotent and may still offer authentication.
