...
| Claude Desktop | macOS desktop runtime inspected 2026-07-15 (internal CCD 2.1.209 in logs) | Desktop rejected a remote HTTP entry in `claude_desktop_config.json` before any network request; Anthropic's documented route for remote MCP is the account-level custom connector UI | CLI 0.1.1 removes the obsolete local Brian entry and directs the user to `https://claude.ai/customize/connectors` with the canonical URL | Connector creation and authenticated MCP use not yet run |
